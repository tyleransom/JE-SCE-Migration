...
log using "SCE_mobility_refb_full_intvws_noSepC4_p01_SepDec_noP0_samecity.log", replace
...
*------------------------------------------------------------------------------
include createchoicedata_p2_0_1_samecity_SepDec
...
append using `m36g'
append using `m36h'
* add in Dec 2019 neighborhood choice data
...
replace maxchoicenum = maxchoicenum/2 // divide by two since each scenario has two probabilities, but I just want to see how many scenarios there were
...
preserve
    ...
    drop if !firstObs
    save ${datapath}UniqueIDsSepDec, replace
restore
...
l scuid choicenum svyblk scennum altnum ratio income crime mvcost taxes norms family dist size homecost in 1/16, sepby(scuid)
...
l scuid choicenum svyblk scennum altnum ratio income crime mvcost taxes norms family dist size homecost schqual samecity copyhome moved in 1/16, sepby(scuid)
...
save ${temppath}alldata_refb_full_intvws_noSepC4_p01_SepDec_noP0_samecity, replace
...
*------------------------------------------------------------------------------
global Suffix = "RefBFullIntvwsNoSepC4p01SepDecNoP0SameCitybswtp"

sum ratio `scenarios', sep(0)

sum ratio `scenarios' if wave==1, sep(0)
sum ratio `scenarios' if wave==1 & altnum==1, sep(0)
sum ratio `scenarios' if wave==1 & altnum==3, sep(0)

sum ratio `scenarios' if wave==2, sep(0)
sum ratio `scenarios' if wave==2 & altnum==1, sep(0)
sum ratio `scenarios' if wave==2 & altnum==3, sep(0)
...
include groupestimation_samecity_inclDec_newWTP_bswtp

* IFE
*qreg ratio `scenarios' i.prefgroup
